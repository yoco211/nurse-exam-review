...
- AI 出題前に現在の分野・難易度・出題数を表示
- AI 生成に失敗した場合は既存の問題を残し、再試行用のエラー表示を出す
- 答題記録、正答率、苦手問題、AI 生成済み問題をブラウザの `localStorage` に保存
- 苦手リストから問題をクリックして復習画面へ戻れる
- 進捗リセット時は確認ダイアログを表示
...
後端は CORS の許可元を制限し、簡易的な IP ベースのレート制限を行います。

- 既定の許可元: `https://yoco211.github.io`、ローカル確認用の `127.0.0.1` / `localhost`
- `ALLOWED_ORIGINS`: 追加で許可する Origin をカンマ区切りで指定
- `AI_RATE_LIMIT_PER_MINUTE`: 1 分あたりの上限。既定値は `20`、`0` 以下で無効化
